...
control_combined_plot <- ggplot(c.index, aes(x = elevation, y = seedset, fill = elevation)) +
  ...
  labs(x = "Elevation (m a.s.l.)", y = "Natural seed set") +
...
pl_plot <- ggplot(c.index, aes(x = elevation, y = PL.index, fill = elevation)) +
  geom_violin(trim = TRUE, scale = "width", width = 0.7) +
  geom_jitter(width = 0.15, size = 1.5, alpha = 0.6, color = "black") +
  labs(x = "Elevation (m a.s.l.)", y = "Pollen limitation") +
  scale_fill_manual(values = elev_colors) +
...
ao_plot_base <- ggplot(ao.index,
  ...
  geom_violin(trim = TRUE, scale = "width", width = 0.7) +
  geom_jitter(width = 0.15, size = 1.5, alpha = 0.6, color = "black") +
  labs(x = "Elevation (m a.s.l.)", y = "Autogamy") +
  scale_fill_manual(values = elev_colors) +
...
go_plot_base <- ggplot(go.index,
  ...
  geom_violin(trim = TRUE, scale = "width", width = 0.7) +
  geom_jitter(width = 0.15, size = 1.5, alpha = 0.6, color = "black") +
  labs(x = "Elevation (m a.s.l.)", y = "Geitonogamy") +
  scale_fill_manual(values = elev_colors) +
...
sx_med <- median(dat$sx, na.rm = TRUE)
sy_med <- median(dat$sy, na.rm = TRUE)
...
newdat_vis <- tibble(
  x  = x_seq,    # z-scored visitation (predictor in model)
  sx = sx_med,
  y  = 0,        # fmorphospecies richness at its mean (z-scale)
  sy = sy_med,
  species = NA
)
...
vis_plot_2 <- ggplot(plot_df_vis, aes(x = x_visitors_per_min, y = Estimate)) +
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5),
              fill = "#d4e9ff", alpha = 0.7, colour = NA) +
  geom_line(size = 1.1, colour = "black") +
  geom_point(data = dat_plot_vis,
  ...
  scale_y_continuous(
    trans  = log1p_trans(),
    breaks = c(0, 10, 50, 250, 1000, 5000, 20000),
    labels = label_comma(big.mark = ","),
    name   = "Natural seed set"
  ) +
  theme_minimal(base_size = 14) +
...
###*
###*
###* FIG 4B - PLOT SEEDSET ON MORPHOSPECIES RICHNESS
# from flowering.visited, before summarising to final.table
m   <- bayesian_seedset_mean_corrected_scaled_13_2_zinb_k_opt
dat <- c.pl.final.table.4

morpho_mean <- mean(dat$mean.morpho, na.rm = TRUE)
morpho_sd   <- sd(dat$mean.morpho,   na.rm = TRUE)

# use 0 as the lower bound for x, plus the observed max
morpho_range <- range(c(0, dat$mean.morpho), na.rm = TRUE)
morpho_seq   <- seq(morpho_range[1], morpho_range[2], length.out = 300)

# build newdata on ORIGINAL scale, convert to z for the model
newdat_morpho <- tibble(
  x  = 0,  # visitation at mean (z-scale)
  sx = sx_med,
...
pred_morpho <- fitted(
  m,
  newdata    = newdat_morpho,
  re_formula = NA,
  summary    = TRUE
) |> as.data.frame()

plot_df_morpho <- bind_cols(newdat_morpho, pred_morpho) %>%
  mutate(morpho_richness = morpho_seq)   # directly use the grid from 0 to max


dat_plot_morpho <- c.pl.final.table.4 %>%
  mutate(
    morpho_richness = mean.morpho   # original morpho richness per species×elevation
  )

morpho_plot <- ggplot(plot_df_morpho, aes(x = morpho_richness, y = Estimate)) +
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5),
              fill = "#d4e9ff", alpha = 0.7, colour = NA) +
  geom_line(size = 1.1, colour = "black") +
  geom_point(data = dat_plot_morpho,
             aes(x = morpho_richness, y = mean_seedset_round),
             inherit.aes = FALSE,
             size = 2.2, alpha = 0.85) +
  scale_x_continuous(
    name   = "Morphospecies richness",
    breaks = pretty(dat_plot_morpho$morpho_richness),
    expand = expansion(mult = c(0, 0.02))
  ) +
  scale_y_continuous(
    name   = "Natural seed set",
...
###* MERGE AND LABEL PLOTS
# 1) Add panel tags
vis_plot_tagged_13 <- vis_plot_2 +
  labs(tag = "A)") +
  theme(plot.tag = element_text(size = 18, face = "bold"))

morpho_plot_tagged_13 <- morpho_plot +
  labs(tag = "B)") +
  theme(plot.tag = element_text(size = 18, face = "bold"))
